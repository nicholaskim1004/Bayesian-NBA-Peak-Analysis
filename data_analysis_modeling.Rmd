...
```{r}
ggplot(data = nba, aes(x = reorder(League_red, League_red_adj, decreasing=TRUE), fill = reorder(Player.Tier,Tier_ord))) + 
  geom_bar(position = "fill") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(title = "Proportional Distribution of Tiers by League",
       x = "Leagues",
...
bpm_reg <- brm(
    ...
    data = nba,
    family = student(),
    prior = priors, chains = 1, iter = 5000
)
...
ggplot(data = tier_info, aes(x = reorder(League,League_ord,decreasing=TRUE), y = Count, fill = Tier_ord)) + 
  geom_bar(stat = "identity", position = "fill") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
```{r}
ggplot(data = tier_info, aes(x = Pick_bin, y = Count, fill = Tier_ord)) + 
  geom_bar(stat = "identity", position = "fill") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
```{r}
ggplot(data = subset(tier_info,League == "Top D1"), aes(x = Pick_bin, y = Count, fill = Tier_ord)) + 
  geom_bar(stat = "identity", position = "fill") +
  theme(axis.text.x = element_text(angle = 45, hjust = 1))
```
