...
nba[nba$Player=="Mãozinha Pereira",12] <- "Cearense"
...
nba[(nba$College == "Nebraska")|(nba$College == "Nebraska-Lincoln"),12] <- "Nebraska"
nba[(nba$College=="St. Mary's")|(nba$College=="St. Mary's College"),12] <- "St. Mary's"

nba[nba$Player=="Alex Sarr",12] <- "Perth"


nba[nba$College=="Capital City Go-Go",12] <- "New Zealand Breakers"
nba[nba$College=="Barcelona, Spain",12] <- "Olympiacos"
nba[nba$College=="Huntington",12] <- "Louisville"
nba[nba$College=="Edgewater HS (FL)",12] <- "IMG Academy (FL)"
nba[nba$College=="Josh Giddey",12] <- "Adelaide"
nba[nba$College=="Rockingham Flames (Aus.)",12] <- "Perth"
nba[nba$College=="Serbia",12] <- "Mega Basket"
nba[nba$College=="Winnipeg Sea Bears",12] <- "Louisiana State"
...
```
Creating a country column which stores what country the league they played is based
```{r}
france <- c("ADA Blois","ASVEL","Cholet","Elan Chalon","INSEP","JL Bourg-en-Bresse","Le Mans","Metropolitans 92","Pau Orthez","Poiteirs Basket 86","Rouen, France","Strasbourg IG")

aus <- c("Adelaide","Australian Institute of Sport","Brisbane","Brose Bamberg","Illawarra","Perth","Sydney")

turkey <- c("Anadolus Efes","Besiktas","Bursaspor","Crvena zvezda","Fenerbachce")

canada <- c("Athlete Institute Prep","Vanier")

germany <- c("Bayern Munich","Braunschweig","Ratiopharm","Riesen Ludwigsburg")

spain <- c("Baloncesto Fuenlabrada","Baskonia","Bibao","Cajasol Sevilla","Estudiantes","FC Barcelona","Gran Canaria","Murcia","Real Madrid","Ricoh Manresa","San Pablo Burgos","Zaragoza")

brazil <- c("Cearense","Flamengo","Minas","Pinheiros")

solvenia <- c("Cedevita","Union Olimpija")

russia <- c("CSKA Moscow")

serbia <- c("Bekio","Mega Basket")

greece <- c("Filathlitikos","Olympiacos","Panathiaikos")

china <- c("Guangdong","Guangzhou Loong Lions")

lithuania <- c("Lietuvos rytas Vilnius","Zalgiris")

israel <- c("Maccabi Tel Aviv")

senegal <- c("NBA Global Academy")

italy <- c("Olimpia Milano","Victoria Liberta")

ukraine <- c("SK Cherkasy Monkey")

japan <- c("Yokohama B-Corsairs (Japan)")

league_country <- list()

for (i in 1:nrow(nba)){
  if (nba$College[i] %in% france){
    league_country[i] = "France"
  }
  else if (nba$College[i] %in% aus){
    league_country[i] = "Australia"
  }
  else if (nba$College[i] %in% turkey){
    league_country[i] = "Turkey"
  }
  else if (nba$College[i] %in% canada){
    league_country[i] = "Canada"
  }
  else if (nba$College[i] %in% germany){
    league_country[i] = "Germany"
  }
  else if (nba$College[i] %in% spain){
    league_country[i] = "Spain"
  }
  else if (nba$College[i] %in% brazil){
    league_country[i] = "Brazil"
  }
  else if (nba$College[i] %in% solvenia){
    league_country[i] = "Solvenia"
  }
  else if (nba$College[i] %in% russia){
    league_country[i] = "Russia"
  }
  else if (nba$College[i] %in% serbia){
    league_country[i] = "Serbia"
  }
  else if (nba$College[i] %in% greece){
    league_country[i] = "Greece"
  }
  else if (nba$College[i] %in% china){
    league_country[i] = "China"
  }
  else if (nba$College[i] %in% lithuania){
    league_country[i] = "Lithuania"
  }
  else if (nba$College[i] %in% israel){
    league_country[i] = "Israel"
  }
  else if (nba$College[i] %in% senegal){
    league_country[i] = "Senegal"
  }
  else if (nba$College[i] %in% italy){
    league_country[i] = "Italy"
  }
  else if (nba$College[i] %in% ukraine){
    league_country[i] = "Ukraine"
  }
  else if (nba$College[i] %in% japan){
    league_country[i] = "Japan"
  }
  else{
    league_country[i] = "USA"
  }
}

nba$Country <- factor(unlist(league_country))
```
...
```
```{r}
country_mapping <- data.frame(country_name = unique(nba$Country),
                              factor_id = as.numeric(unique(nba$Country)))
```
...
ggplot(ppc_df, aes(x = sim_mean, y = observed)) +
  ...
  labs(title = "Variational Plot of Actual Superstar Count per College vs Sim",
       x = "Predicted # superstars (mean ± 90% CI)", y = "Observed # superstars")
```
